...
import { dirHome, dirSched, dirAthletes, dirCoaches, dirTimecards, dirMsgs, dirBilling, dirSubs, dirInjuries, dirAttendance, dirDocuments, dirRoutines, dirAnalytics, dirCertCourses } from './views-director.js?v=800';
...
async function loadAll(){
  ...
  try{
    const results=await Promise.allSettled([
      ...
      getDocs(query(collection(db,'charges'),where('gymId','==',gid))),
    ]);
    const safe=i=>results[i].status==='fulfilled'?results[i].value:{docs:[]};
    const[clsS,athS,coaS,tcS,subS,injS,msgS,noteS,docS,routS,mkS,chgS]=Array.from({length:12},(_,i)=>safe(i));
    ...
    APP.allCharges=chgS.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));
...
function showApp(){
  hide('loadingScreen');hide('authScreen');hide('obScreen');
  if(APP.role==='coach'){
    const activeTC=(APP.myTimecards||[]).find(t=>t.status==='active'&&t.coachId===APP.user?.uid);
    if(activeTC){APP.clockedIn=true;APP.clockSessionId=activeTC.id;APP.clockInTime=activeTC.clockIn?new Date(activeTC.clockIn):new Date();if(activeTC.classId){const cls=APP.allClasses.find(c=>c.id===activeTC.classId);if(cls){APP.selectedClass={...cls,athleteObjects:APP.allAthletes.filter(a=>(cls.athletes||[]).includes(a.id))};APP.activeSkillClass=activeTC.classId;}}}
  }
  $('appShell').style.display='flex';initApp();if(APP.role==='coach')startNotificationWatcher();if(APP.profile?.firstLogin)setTimeout(()=>K.openModal('changePwModal'),800);
}
...
function initApp(){
  ...
  // Director sidebar live stats
  const sbStats=$('sbStats');
  if(sbStats){
    if(r==='director'){
      const rev=(APP.allAthletes||[]).reduce((s,a)=>s+(a.tuitionAmount||185),0);
      const active=(APP.allTimecards||[]).filter(t=>t.status==='active');
      const overdue=(APP.allAthletes||[]).filter(a=>a.tuitionStatus==='overdue').length;
      const unread=(APP.messages||[]).filter(m=>m.fromId!==APP.user?.uid&&m.fromId!=='system'&&!m.read).length;
      sbStats.style.display='block';
      sbStats.innerHTML=`<div style="padding:10px 14px;border-top:1px solid rgba(181,153,106,0.12);">
        <div style="font-family:'Barlow Condensed',sans-serif;font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:rgba(181,153,106,0.4);margin-bottom:8px;">Gym Overview</div>
        <div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="font-size:11px;color:rgba(250,250,248,0.4);">Monthly Rev</span><span style="font-size:11px;font-weight:700;color:var(--gold);">$${rev.toLocaleString()}</span></div>
        <div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="font-size:11px;color:rgba(250,250,248,0.4);">Athletes</span><span style="font-size:11px;font-weight:700;color:rgba(250,250,248,0.7);">${(APP.allAthletes||[]).length}</span></div>
        ${active.length>0?`<div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="font-size:11px;color:rgba(250,250,248,0.4);">Clocked In</span><span style="font-size:11px;font-weight:700;color:#5EC85E;display:flex;align-items:center;gap:4px;"><span style="width:6px;height:6px;border-radius:50%;background:#5EC85E;display:inline-block;"></span>${active.length}</span></div>`:''}
        ${overdue>0?`<div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="font-size:11px;color:rgba(250,250,248,0.4);">Overdue</span><span style="font-size:11px;font-weight:700;color:#FF8A80;">${overdue}</span></div>`:''}
        ${unread>0?`<div style="display:flex;justify-content:space-between;"><span style="font-size:11px;color:rgba(250,250,248,0.4);">Unread Msgs</span><span style="font-size:11px;font-weight:700;color:var(--gold);">${unread}</span></div>`:''}
      </div>`;
    } else {
...
async saveNewClass(){const name=val('nc_name').trim(),day=val('nc_day');const tHH=val('nc_time_hh')||'9',tMM=val('nc_time_mm')||'00',tAP=val('nc_time_ap')||'AM';const time=`${tHH}:${tMM} ${tAP}`;const duration=val('nc_duration')||'60';const coachId=val('nc_coach'),cap=+val('nc_cap')||8,level=val('nc_level')||'Level 1',notes=val('nc_notes').trim();const coachObj=APP.allCoaches.find(c=>c.id===coachId);if(!name){toast('Class name required','warn');return;}try{const data={name,day,time,duration,coachId,coachName:coachObj?.name||'TBD',coaches:coachId?[coachId]:[],cap,level,notes,athletes:[],gymId:APP.gymId||null,createdAt:new Date().toISOString()};const ref=await addDoc(collection(db,'classes'),data);APP.allClasses.push({id:ref.id,...data});K.closeModal('addClassModal');toast('✓ Class added!');render();}catch(e){toast('⚠️ '+e.message,'warn');}},
async saveEditClass(id){const name=val('ec_name').trim(),day=val('ec_day');const tHH=val('ec_time_hh')||'9',tMM=val('ec_time_mm')||'00',tAP=val('ec_time_ap')||'AM';const time=`${tHH}:${tMM} ${tAP}`;const level=val('ec_level')||'Level 1',cap=+val('ec_cap')||8,notes=val('ec_notes').trim();const duration=val('ec_duration')||'60';const selCoaches=[...document.querySelectorAll('input[name="ec_coach"]:checked')].map(c=>c.value);const first=APP.allCoaches.find(c=>c.id===selCoaches[0]);const coachName=selCoaches.length===0?'TBD':selCoaches.length===1?(first?.name||'TBD'):`${first?.name||'Coach'} +${selCoaches.length-1}`;if(!name){toast('Class name required','warn');return;}try{const upd={name,day,time,duration,level,cap,notes,coaches:selCoaches,coachId:selCoaches[0]||null,coachName};await setDoc(doc(db,'classes',id),upd,{merge:true});const cls=APP.allClasses.find(c=>c.id===id);if(cls)Object.assign(cls,upd);K.closeModal('editClassModal');toast('✓ Updated!');render();}catch(e){toast('⚠️ '+e.message,'warn');}},
...
function buildEditClassForm(cls){
  const timeParts=(cls.time||'').match(/^(\d+):(\d+)\s*(AM|PM)$/i)||['','9','00','AM'];
  const tHH=timeParts[1]||'9',tMM=timeParts[2]||'00',tAP=(timeParts[3]||'AM').toUpperCase();
  ...
  <div class="g2" style="margin-bottom:0;"><div class="fg"><label class="fl">Day</label><select class="fs" id="ec_day">${['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'].map(d=>`<option value="${d}"${cls.day===d?' selected':''}>${d}</option>`).join('')}</select></div>
  <div class="fg"><label class="fl">Time *</label><div style="display:flex;gap:6px;"><input class="fi" id="ec_time_hh" value="${tHH}" style="width:60px;text-align:center;" maxlength="2"><span style="align-self:center;font-weight:700;color:var(--t3);">:</span><input class="fi" id="ec_time_mm" value="${tMM}" style="width:60px;text-align:center;" maxlength="2"><select class="fs" id="ec_time_ap" style="width:80px;"><option ${tAP==='AM'?'selected':''}>AM</option><option ${tAP==='PM'?'selected':''}>PM</option></select></div></div></div>
  ...
  <div class="fg"><label class="fl">Duration</label><select class="fs" id="ec_duration"><option value="30" ${cls.duration==='30'?'selected':''}>30 min</option><option value="45" ${cls.duration==='45'?'selected':''}>45 min</option><option value="60" ${!cls.duration||cls.duration==='60'?'selected':''}>1 hour</option><option value="75" ${cls.duration==='75'?'selected':''}>1h 15m</option><option value="90" ${cls.duration==='90'?'selected':''}>1h 30m</option><option value="120" ${cls.duration==='120'?'selected':''}>2 hours</option></select></div>
